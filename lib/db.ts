...
export async function mirrorSignup(
  publicId: string,
  opts?: { email?: string; firstName?: string; createdAt?: string; source?: string },
): Promise<string | null> {
  ...
  // Mangler signup_source-kolonnen endnu (migration ikke kørt), må kilden
  // ALDRIG vælte selve tilmeldingen — prøv igen uden den. Gælder KUN den
  // specifikke ukendt kolonne-fejl (PGRST204 / Postgres 42703); alle andre
  // fejl (timeout, RLS osv.) skal kaste som før, uden at miste kilden.
  // TODO: fjern denne fallback når signup_source-kolonnen er bekræftet i prod.
  const columnMissing =
    error &&
    ((error as { code?: string }).code === 'PGRST204' ||
      (error as { code?: string }).code === '42703' ||
      /signup_source/.test(error.message ?? ''))
  if (columnMissing && 'signup_source' in row) {
    console.error('mirrorSignup: signup_source column missing, retrying without it', error?.message)
    delete row.signup_source
